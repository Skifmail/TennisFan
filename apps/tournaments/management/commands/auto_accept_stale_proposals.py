"""
Автоматически подтвердить заявки на результат матча, ожидающие подтверждения более 3 часов.

Если один игрок отправил результат на подтверждение второму, у второго есть 3 часа на ответ.
По истечении 3 часов заявка считается подтверждённой системой (результат применяется к матчу).
...
PROPOSAL_AUTO_ACCEPT_HOURS = 3


class Command(BaseCommand):
    help = (
        "Найти заявки на результат матча (status=PENDING) старше 3 часов и применить их, "
    ...
    def handle(self, *args, **options):
        ...
        if not proposals:
            self.stdout.write("Нет заявок на авто-подтверждение (старше 3 ч).")
            return
        total = 0
        for proposal in proposals:
            try:
                ...
                msg = (
                    "Результат матча автоматически подтверждён по истечении 3 часов "
