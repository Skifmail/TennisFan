...
def populate_allowed_categories(apps, schema_editor):
    ...
    TournamentAllowedCategory = apps.get_model(
        "tournaments", "TournamentAllowedCategory"
    )
    for t in Tournament.objects.all():
        if t.category:
            TournamentAllowedCategory.objects.get_or_create(
                tournament=t, defaults={"category": t.category}
            )
