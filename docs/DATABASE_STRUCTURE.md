...
Разделы правил (slug, title, body). Текст по умолчанию берётся из HTML-шаблонов
в `templates/core/rules_*.html` (см. `apps.content.rules_defaults`); миграция
и админка заполняют пустой `body`, чтобы контент можно было править в админке.
