...
Dokploy **не ставится** через `git clone` и `docker compose`: в репозитории нет готового `docker-compose.yml`. Установка делается **официальным скриптом**, который поднимает сервисы через Docker Swarm.

### 2.1 Установка скриптом (рекомендуется)

На сервере (root или sudo) выполните:
...
- поднимет PostgreSQL, Redis и сам Dokploy как сервисы Swarm;
- поднимет Traefik (порты 80, 443);
- опубликует веб-интерфейс на порту **3000**.

Перед запуском убедитесь, что свободны порты **80**, **443** и **3000**:

```bash
ss -tulnp | grep -E ':80 |:443 |:3000 '
```

Если что-то занято — остановите сервис или выберите другой способ установки (см. [Manual Installation](https://docs.dokploy.com/docs/core/manual-installation)).

После установки подождите 15–30 секунд и откройте в браузере:

**http://IP_ВАШЕГО_СЕРВЕРА:3000**

При первом заходе создайте учётную запись супер-админа (логин и пароль).
