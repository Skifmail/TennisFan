...
(function () {
    ...
    function updateGenderOptions() {
        ...
        console.log("Updating gender options, variant:", variant);

        // Получаем все опции
        const options = genderSelect.querySelectorAll("option");
        let mixedOptionFound = false;
        
        options.forEach(function(option) {
            // Для одиночных турниров скрываем "Микст" (mixed)
            if (option.value === "mixed") {
                mixedOptionFound = true;
                console.log("Found mixed option, variant is doubles:", variant === VARIANT_DOUBLES);
                
                if (variant === VARIANT_DOUBLES) {
        ...
        if (!mixedOptionFound) {
            console.log("Mixed option not found in gender select");
        }
    }
